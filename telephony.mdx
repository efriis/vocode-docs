...
## Environments
To proceed, you’ll need to grab credentials from these providers and set them in the Vocode environment.
```python
...
## Telephony Server
This is a sample implementation of a telephony server using the Vocode library to
handle inbound and outbound calls. The server is built using FastAPI and utilizes 
...
1. Add `twilio_config` to the `TelephonyServer` constructor. This will be used to authenticate with Twilio.
```python
telephony_server = TelephonyServer(
    base_url=BASE_URL,
    config_manager=config_manager,
...
2. Add `twilio_config` to the `OutboundCall` constructor. This will be used to authenticate with Twilio.
```python
outbound_call = OutboundCall(
    base_url=BASE_URL,
    to_phone="+11234567890",
...
```
Replace the `to_phone` with the number you want to call and the `from_phone` with the number you want to call from.
...
Make note of the public URL provided by Ngrok (e.g., `https://abcd1234.ngrok.io`). Update the `BASE_URL` variable in the code with your Ngrok URL.
